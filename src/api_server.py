...
@app.get("/tours/{tour_id}/audio")
async def get_tour_audio_urls(tour_id: str, language: str = "en"):
    """
    Get audio URLs for all POIs in a tour.

    Simplified endpoint for client apps - returns just the audio information
    without requiring full tour access.
    ...
    Returns:
        List of POIs with audio URLs

    Example:
        GET /tours/rome-tour-20260320-175540-6b0704/audio?language=zh-tw
    ...
    """
    import json
    from utils import poi_name_to_id, check_audio_exists, get_poi_audio_url, normalize_language_code
    ...
    try:
        ...
        if not tour_file.exists():
            tour_file = tour_path / "tour_zh-tw.json"  # Fallback
        if not tour_file.exists():
            tour_file = tour_path / "tour_en.json"  # Second fallback
        ...
        for day_data in tour_data.get('itinerary', []):
            ...
            for poi_data in day_data.get('pois', []):
                ...
                audio_available = check_audio_exists(city, poi_name, language)
                audio_url = None

                if audio_available:
                    audio_url = get_poi_audio_url(city, poi_name, language)
        ...
        logger.error(f"Error getting tour audio URLs: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error getting audio URLs: {str(e)}"
        )
