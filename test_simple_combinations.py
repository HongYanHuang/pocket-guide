"""
Simple test: Find which constraint conflicts with combo tickets
"""
import sys
...
print("Testing: Time Windows + Combo Tickets (NO precedence)")
print("=" * 80)
...
def noop_prec(*args, **kwargs):
    print("  [DISABLED] Precedence constraints")
...
if status in ['OPTIMAL', 'FEASIBLE']:
    ...
    print("   → Precedence is the culprit!")
else:
    print("❌ Time Windows + Combo Tickets = INFEASIBLE")
    print("   → Time Windows conflict with combo tickets (precedence not the issue)")
